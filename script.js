...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ================================================================
     ...
     ================================================================ */
  if (typeof TREN_DATA === "undefined") return; // No estamos en tren.php
  ...
  // ---- Iniciar viaje desde el botón ----
  window.iniciarViaje = function () {
    if (!boardingScreen) return;

    // Efecto sonido visual (shake del botón)
    ...
    // Transición de salida de embarque
  ...
  // ---- Generar paisaje procedural en la ventana ----
  function generarPaisaje() {
    ...
    // Cielo con estrellas/nubes
    html += `<div style="
      position:absolute; top:0; left:0; width:100%; height:50%;
      background: linear-gradient(to bottom, #0a0a12, ${colores.horizonte});
    "></div>`;
    ...
    html += `<div style="
      ...
      background: radial-gradient(circle at 35% 35%, #f5deb3, #c9a66b);
    ...
    montañas.forEach(([x, col]) => {
      ...
      html += `<div style="
        position:absolute; left:${x}px; bottom:35%;
    ...
    html += `<div style="
      ...
      background: linear-gradient(to bottom, #1a1210, #0a0a08);
    "></div>`;

    // Elementos de paisaje: árboles, postes, edificios
    const elementos = [];
    for (let i = 0; i < 60; i++) {
      ...
      if (tipo < 0.4) {
        ...
        elementos.push(`
          <div style="position:absolute; left:${x}px; bottom:38%; width:${w}px; height:${h}px;
            background: radial-gradient(ellipse 50% 60% at 50% 40%, ${colores.arbol}, #1a2a1a);
            border-radius: 50% 50% 20% 20%;"></div>
          <div style="position:absolute; left:${x + w/2 - 2}px; bottom:37%;
            width:4px; height:15px; background:#1a1008;"></div>
        `);
      } else if (tipo < 0.6) {
        // Poste de telégrafo
        const h = 50 + Math.random() * 30;
        elementos.push(`
          <div style="position:absolute; left:${x}px; bottom:38%;
            width:3px; height:${h}px; background:#1a1210;"></div>
          <div style="position:absolute; left:${x - 8}px; bottom:${38 + (h/5)}%;
            width:18px; height:2px; background:#1a1210;"></div>
        `);
      } else if (tipo < 0.75) {
        // Edificio lejano
        ...
        elementos.push(`
          <div style="position:absolute; left:${x}px; bottom:38%;
            width:${bw}px; height:${bh}px; background:${colores.edificio};"></div>
        `);
      }
    }
    html += elementos.join("");

    // Riel (línea de ferrocarril en primer plano)
    html += `<div style="
      position:absolute; bottom:20%; left:0; width:100%; height:4px;
      background: repeating-linear-gradient(to right, var(--ocre-oscuro) 0px, var(--ocre-oscuro) 30px, transparent 30px, transparent 50px);
      opacity: 0.6;
    "></div>`;
  ...
  // ---- Activar lluvia suave ----
  function activarLluvia() {
    const rain = document.getElementById("rainOverlay");
    if (!rain) return;

    // Lluvia aleatoria leve
    if (Math.random() > 0.5) return; // 50% de probabilidad
  ...
  function renderParada(parada, index) {
    ...
    // Datos
    const datosHTML = (parada.datos || []).map(d =>
      `<div class="parada-dato">${d}</div>`
    ).join("");
